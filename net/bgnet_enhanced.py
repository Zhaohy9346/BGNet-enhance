...
sys.path.append('..')
from enhancement.enhancement_net import EnhancementModule
...
class Net_Enhanced(nn.Module):
    """BGNet with Enhancement Module"""

    def __init__(self, use_enhancement=True):
        super(Net_Enhanced, self).__init__()

        # Enhancement module (新增)
        self.use_enhancement = use_enhancement
        self.enhancement = EnhancementModule(use_enhancement=use_enhancement)

        # Original BGNet components
    ...
    def forward(self, x):
        """
        Args:
            x: (B, 3, H, W) normalized image [0, 1]
        Returns:
            o3, o2, o1, oe: predictions
            enhanced_img: enhanced image (if use_enhancement=True)
            filter_params: filter parameters (if use_enhancement=True)
        """
        ...
        if self.use_enhancement:
            ...
            # Apply enhancement
            enhanced, filter_params = self.enhancement(x_denorm)

            # Re-normalize for ResNet
            x = (enhanced - mean) / std
        else:
            enhanced = None
            filter_params = None

        # Step 2: Original BGNet forward
        ...
        return o3, o2, o1, oe, enhanced, filter_params


# 保持原始Net类以兼容旧代码
class Net(nn.Module):
    """Original BGNet (unchanged)"""
