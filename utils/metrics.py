...
@numba.jit(nopython=True)
def _divide_with_xy(pred: np.ndarray, gt: np.ndarray, x, y):
    # 修改返回类型注解为 tuple，兼容 Python 3.8
    h, w = gt.shape
    area = h * w
...
class WeightedFmeasure(object):
    ...
    @staticmethod
    def _matlab_style_gauss2D(shape: Tuple[int, int] = (7, 7), sigma: int = 5) -> np.ndarray:
        """
